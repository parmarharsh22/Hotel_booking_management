 ...
 * GET AVAILABLE ROOMS
 * Checks:
 * 1. Room physical status = AVAILABLE (room_status_id = 1)
 * 2. Not already in a confirmed/checked-in booking for those dates (via booking_rooms)
 * NO room_availability table used — date availability is purely from booking_rooms overlap
 */
    ...
    const [rooms]: any = await db.query(
        `SELECT
         ...
         WHERE r.hotel_id       = ?
           ...
           -- not in an active booking for overlapping dates
           AND r.room_id NOT IN (
               SELECT br.room_id
               FROM booking_rooms br
               JOIN bookings b ON br.booking_id = b.booking_id
               WHERE b.hotel_id          = ?
                 AND b.booking_status_id NOT IN (5)
                 AND b.checkin_date       < ?
                 AND b.checkout_date      > ?
           )
...
export const createBooking = async (connection: any, data: {
    ...
    const [result]: any = await connection.query(
        ...
        VALUES (?,?,?,?,?,?,?,?,?,?,?)`,
        [
            data.hotel_id, data.user_id,
            data.booking_status_id, data.booking_source_id,
...
/**
 * CREATE PAYMENT
 * Created as PENDING — updated to SUCCESS/FAILED after payment
 */
